...
import { type InformeData, type InformeCombinado, type InformeCompleto, type SaldoCuentaBancaria, type PagoDetalladoInforme } from "@/lib/database"
...
function drawPagosTable(
  doc: jsPDF,
  pagos: PagoDetalladoInforme[],
  startY: number,
  margin: number,
  availableWidth: number,
  pageHeight: number,
  ...
  const pagosTableWidth = availableWidth

  // Header - AZUL
  doc.setFillColor(...colors.blue)
  doc.rect(tableX, y, pagosTableWidth, 9, "F")
  ...
  let colX = tableX + 4
  pagosHeaders.forEach((header, i) => {
    const align = i >= 4 ? "right" : "left"
    const textX = i >= 4 ? colX + pagosColWidths[i] - 4 : colX
    doc.text(header, textX, y + 6, { align })
    colX += pagosColWidths[i]
  })
  ...
  for (let i = 0; i < pagos.length; i++) {
    ...
    if (y > pageHeight - 25) {
      ...
      colX = tableX + 4
      pagosHeaders.forEach((header, idx) => {
        const align = idx >= 4 ? "right" : "left"
        const textX = idx >= 4 ? colX + pagosColWidths[idx] - 4 : colX
        doc.text(header, textX, y + 6, { align })
        colX += pagosColWidths[idx]
      })
      y += 9
      doc.setFont("helvetica", "normal")
    }
    ...
    if (i % 2 === 0) {
      doc.setFillColor(...colors.background)
      doc.rect(tableX, y, pagosTableWidth, 7, "F")
    }
    ...
    doc.text(fecha, colX, rowY)
    colX += pagosColWidths[0]

    // Apto
    doc.setFont("helvetica", "bold")
    doc.text(pago.apartamentoNumero, colX, rowY)
    doc.setFont("helvetica", "normal")
    colX += pagosColWidths[1]

    // Tipo
    doc.setTextColor(pago.tipoOcupacion === "PROPIETARIO" ? colors.blue[0] : 147,
                     pago.tipoOcupacion === "PROPIETARIO" ? colors.blue[1] : 51,
                     pago.tipoOcupacion === "PROPIETARIO" ? colors.blue[2] : 234)
    const tipoLabel = pago.tipoOcupacion === "PROPIETARIO" ? "Propietario" : "Inquilino"
    doc.text(tipoLabel, colX, rowY)
    colX += pagosColWidths[2]

    // Nombre
    doc.setTextColor(...colors.primary)
    let nombre = pago.contactoNombre
      ? `${pago.contactoNombre} ${pago.contactoApellido || ''}`.trim()
      : "-"
    const maxNombreWidth = pagosColWidths[3] - 4
    while (doc.getTextWidth(nombre) > maxNombreWidth && nombre.length > 3) {
      nombre = nombre.slice(0, -4) + "..."
    }
    doc.text(nombre, colX, rowY)
    colX += pagosColWidths[3]

    // Monto total
    doc.setTextColor(...colors.positive)
    doc.text(formatCurrency(pago.monto), colX + pagosColWidths[4] - 4, rowY, { align: "right" })
    colX += pagosColWidths[4]

    // Monto G.C.
    doc.setTextColor(...colors.primary)
    doc.text(formatCurrency(pago.montoGastoComun), colX + pagosColWidths[5] - 4, rowY, { align: "right" })
    colX += pagosColWidths[5]
  ...
  // Fila de totales
  doc.setDrawColor(...colors.primary)
  doc.line(tableX, y, tableX + pagosTableWidth, y)
  y += 1

  doc.setFillColor(...colors.light)
  doc.rect(tableX, y, pagosTableWidth, 8, "F")

  doc.setFont("helvetica", "bold")
  doc.setTextColor(...colors.primary)
  doc.text("TOTAL", tableX + 4, y + 5.5)

  // Calcular posiciones de totales
  let totalColX = tableX + 4 + pagosColWidths[0] + pagosColWidths[1] + pagosColWidths[2] + pagosColWidths[3]

  doc.setTextColor(...colors.positive)
  doc.text(formatCurrency(totalMonto), totalColX + pagosColWidths[4] - 4, y + 5.5, { align: "right" })
  totalColX += pagosColWidths[4]

  doc.setTextColor(...colors.primary)
  doc.text(formatCurrency(totalGC), totalColX + pagosColWidths[5] - 4, y + 5.5, { align: "right" })
...
export function generateInformeDetalleMensual(data: InformeCompleto, piePagina?: string, avisoFinal?: string) {
  const avisosActivos = (data.avisos || []).filter((a) => a.activo)
  const doc = new jsPDF("landscape")
  const pageWidth = doc.internal.pageSize.getWidth()
  const pageHeight = doc.internal.pageSize.getHeight()
  ...
  // =====================================================
  // 4. RESUMEN BANCARIO DEL MES ANTERIOR
  // =====================================================
  if (y > pageHeight - 45) {
    doc.addPage("landscape")
    drawPageHeader()
  ...
  // =====================================================
  // 4b. SALDO POR CUENTA BANCARIA DEL MES ANTERIOR
  // =====================================================
  if (data.datosAnterior.saldosPorCuenta && data.datosAnterior.saldosPorCuenta.length > 0) {
    y = drawSaldosPorCuenta(doc, data.datosAnterior.saldosPorCuenta, y, margin, availableWidth, formatCurrencyLocal, pageHeight)
  }
  ...
  // =====================================================
  // 7. DETALLE DE INGRESO - APARTAMENTOS (NUEVO)
  // =====================================================
  if (data.pagosDelMesAnterior && data.pagosDelMesAnterior.length > 0) {
    if (y > pageHeight - 50) {
      doc.addPage("landscape")
      drawPageHeader()
      y = 40
    ...
    y = drawSectionTitle(doc, `DETALLE DE INGRESO - APARTAMENTOS - ${data.mesAnterior.label.toUpperCase()}`, margin, y)
    y = drawPagosTable(doc, data.pagosDelMesAnterior, y, margin, availableWidth, pageHeight, drawPageHeader)
    y += 16
  }
